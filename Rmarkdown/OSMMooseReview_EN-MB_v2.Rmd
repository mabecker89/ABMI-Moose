...
# Set up working directory - collaborators to adjust as needed.
...
the effect of oil and gas extraction has been on the moose population. 

We extracted relevant estimates of moose population state variables, such 
...
there are any trends in time or space, further work is required to calibrate 
density estimates from different sources or methods. Further sampling in coming 
years will also help address these research questions. 
...
+ More detail around reporting of published effect sizes for various impacts and 
disturbances.
...
In this report, the study area is defined as that of Alberta's Oil Sands Region
and all the Wildlife Management Units that intersect it geographically (Figure 1).
A total of 37 WMUs meet this criteria.  
...
The following data sources will be used in this review:
...
+ Data from other camera monitoring efforts
...
In Alberta, aerial surveys have been heavily relied upon to generate estimates
of ungulate population parameters, including moose, at the WMU scale. Prior to
2014, a stratified random block approach was most commonly used; however, the 
Government of Alberta has since adopted distance sampling as the preferred 
aerial survey approach. Distance sampling includes an estimate of detectability, 
which helps to correct for sightability biases inherent to previous methods 
(Peters et al. 2014). This technique accounts for variation in detectability by 
assuming that animals close to the observer are more easily detected than when 
the animal is farther away (Buckland et al. 1993). A recent review found that 
distance sampling, with the sightability correction, likely provides more 
accurate estimates of parameters such as density and with greater efficiency, 
i.e. less flight hours (Peters et al 2014). In this review we report results 
from both eras (1993-2013 using random block, and 2014-present using distance 
sampling) but urge caution in direct comparison between estimates using 
different methods. Without a detection adjustment, random block surveys are more 
likely to have underestimated moose populations.

Other methods that have also been used to study moose in the OSR include GPS 
telemetry (e.g. Boutin et al. 2015), remote camera traps (e.g. Fisher & Burton, 
2018), snowtrack surveys (e.g. Toews et al. 2017), and scat collection (e.g. 
Wasser et al. 2011). 

*To add:*

+ Expand upon other methods used to study moose population parameters. 
...
aerial survey reports from each WMU in the region. 

The average density of moose within WMUs of the OSR between 2014 and 2018 
(the interval over which the Government of Alberta began using the distance 
sampling method) is 0.17 individuals per km$^2$. From 1993 to 2013, when the random 
block/Gasaway method was used, moose density averaged 0.18 individuals per 
km$^2$.

*To add:*

+ Data from 2019 aerial surveys will be available in early May - a total of 5 
WMUs were flown this year. 
...
Figure 3 displays the mean annual moose density across surveyed WMUs from 1993
to 2018. The error bars are one standard error. The same outlier was removed from this
plot. 

```{r include=TRUE, echo=TRUE, message=FALSE, warning=FALSE, eval=TRUE, out.width="100%", fig.cap="Figure 3. Mean annual moose density in WMUs of the Oil Sands Region."}

dens2 <- df_dens2 %>%
  filter(mean_dens < 1.2) %>%
  mutate(Method = as.factor(if_else(Survey_Year > 2013, "Distance", "Random Block"))) %>%
  ggplot(aes(x = Survey_Year, y = mean_dens)) +
  geom_errorbar(aes(ymin = mean_dens - se, ymax = mean_dens + se, 
                    width = 0.1)) +
  geom_point(aes(size = count, color = Method)) +
  scale_x_continuous(breaks = c(1995, 2000, 2005, 2010, 2015)) +
  labs(y = expression(Density~(per~km^2)), x = "",
...
et al. 2016). In northern Alberta, RCs are regularly used to monitor mammal 
populations as they can be relatively easy and cost-effective to deploy and 
allow for spatially explicit analysis of animal densities and occupancy across
the landscape. RCs also have the ability to provide more detailed information
such as sex and age ratios, body condition, and behaviour as well as community
level metrics such as species richness and phenology. In addition, RCs can 
...
+ Additional background regarding the proliferation in the use of 
...
are distributed along a 20 km systematic grid across the province. In the OSR, 
20% of sites are visited each year. The ABMI estimates the density of mammals 
such as moose from RC data using the cumulative residency time (CRT) that the 
species is captured on camera. This method estimates density by assuming that 
animals use (and move across) the sampled area randomly such that the total 
residency time in any given location is a function of the number of animals in a
fixed area [(Huggard, 2018)](https://www.abmi.ca/home/publications/501-550/516).  
...
estimates were calculated using RC data from the years 2015-2017, whereas 
aerial survey density estimates were calculated from a singe year sample within 
the the five year time period. There were fourteen WMUs that we were able to 
obtain moose density estimates from both sources. Table 1 below displays the 
results, and the corresponding confidence intervals in estimates. 
...
This means that only large changes in moose population would be detected with 
statistical confidence at the WMU-scale.
...
Moose density estimates from ABMI's RCs are likely to be biased upwards. The 
first reason for this is a practical element of using RCs: cameras are usually 
placed in small openings so that they are not obscured by vegetation. However, 
moose tend to use these areas preferentially for foraging (at least in the 
summer), leading to overestimates of density. The second reason is that moose 
are also attracted to the cameras. In order to get accurate absolute density 
estimates from RCs, a calibration procedure using other methods (such as aerial 
surveys) will likely be required. However, this RC data can still be useful as 
relative abundance estimates, and be used for habitat modeling (see Section 4.2 
on Distribution) and for long-term trend analysis.
...
Schneider et al. 2000 examined broad scale changes to moose density across 
Northern Alberta using random stratified block aerial surveys. Moose median 
densities increased from 0.25 per km$^2$ in the green zone to 0.4 per km$^2$ in 
the white zone. Moose density was also found to be higher in the foothills and 
mixedwood regions.

*To add:*

+ Description of other published sources of density estimates. 
...
1993 and 2018 (Table 3). Across all WMUs and years, mean lambda was 1.03. As 
...
years but varies by WMU, with growth rates generally decreasing with
latitude (Figure 7). 
...
datatable(data = df_lamb2,
          ...
          colnames = c("WMU", "Lambda",
                       "Number of Surveys"),
          rownames = FALSE,
          caption = "Table 3. Calculations of Lambda in 14 WMUs of the OSR. Includes
          WMUs which have been surveyed at least twice between 1993-2018.",
          width = 500,
          options = list(
...
``` {r include=TRUE, echo=TRUE, message=FALSE, warning=FALSE, eval=TRUE, fig.cap="Figure 7. Moose lambda in WMUs across the OSR."}
...
+ More detail regarding findings of moose survival.
...
+ The intent is to add a section where impact coefficients (size, magnitude) can
be compared across methods. In this section a correlation matrix of available 
data / studies will be developed to faciliate this comparison. 
...
Other studies have also investigated how moose respond to various features in 
the OSR landscape. 

*To add:*

+ Narrative description of other studies (e.g. Toews et al 2017).
...
+ Review documents related to the effect of human activity on moose behaviour
