...
include_graphics("./tmap_png/aoi.png")
...
include_graphics("./tmap_png/dens.png")
...
df_dens_tab <- df_dens_curr %>%
  ...
  mutate(CI_gov = paste(CIL, CIU, sep = ", ")) %>%
  na_if("NA, NA") %>%
  mutate(SampleSize = NA) %>%
  rename(Density_gov = Density,
         Source = Method) %>%
  select(WMU, Source, Survey_Year, SampleSize, Density_gov, CI_gov) %>%
  bind_rows(df_abmi1) %>%
  mutate(Source = str_replace_all(Source, "Distance Sampling", "GoA")) %>%
  arrange(desc(WMU)) %>%
  mutate(WMU = as.character(WMU),
         Density_gov = round(Density_gov, digits = 2),
         Density_abmi = round(Density_abmi, digits = 2),
         CI_gov = )
...
datatable(data = df_dens_tab, filter = "top", rownames = FALSE,
          ...
          colnames = c("WMU", "Source", "Aerial Survey Year", 
                       "ABMI Camera Deployments",
                       "Density - Aerial", "90% CI - Aerial", 
                       "Density - ABMI", "90% CI - ABMI"),
          options = list(pageLength = 10, scrollX = TRUE))

```

```{r include=TRUE, echo=TRUE, message=FALSE, warning=FALSE, eval=TRUE, fig.cap="Density Comparison"}
...
placed in each WMU (Sample Size, in the above table), such that confidence 
...
0.54, and 0.41 animals per km2 in 2015, 2016, and 2017, respectively [^1].
...
include_graphics("./tmap_png/lamb.png")
...
tm_map_demo1 <- ab_wmu_demo %>%
  filter(Response == "CalfCow" | is.na(Response)) %>%
  tm_shape() +
    tm_fill(col = "ratio",
            palette = c1,
            style = "cont",
...
tm_map_demo2 <- ab_wmu_demo %>%
  filter(Response == "MaleFemale" | is.na(Response)) %>%
  tm_shape() +
    tm_fill(col = "colour",
            style = "cont") +
    tm_borders(col = "black") +
...
include_graphics("./tmap_png/demo.png")
