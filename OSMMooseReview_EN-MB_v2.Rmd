...
WMUs that interest the oilsands region.
...
``` {r include = TRUE, echo = TRUE, message = FALSE, warning = FALSE, eval = TRUE, fig.dim = c(10,9)}
...
ab_prov %>%
  ...
  setView(lng = -113.89, lat = 56.53, zoom = 6.5) %>%
  
  # Polygon layers
  addPolygons(color = "#070707", weight = 1, smoothFactor =  0.2, 
              opacity = 2.0) %>%
  
  addPolygons(data = ab_wmu_dens, color = "#070707", weight = 0.8,
              ...
              popup = paste("WMU Name:", 
                            ...
                            "<b>", ab_wmu$WMU, "</b>", "<br>",
                            "Moose Density:", 
                            "<b>", ab_wmu_dens$Density, "</b>", "per km2", 
                            "<br>", 
                            "<br>",
                            "<i>", "Last surveyed in", 
  ...
  # Legend layer
  addLegend(data = ab_wmu_dens, position = "bottomleft", pal = pal_1, bins = 6,
            values = ~Density, 
...
datatable(data = df_lamb2,
          ...
          colnames = c("Wildlife Management Unit", "Lambda",
                       "Number of Surveys"),
          rownames = FALSE,
          width = 500,
          options = list(
...
tmap_arrange(tm_map_demo1, tm_map_demo2)
