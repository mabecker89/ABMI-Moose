...
Wildlife Management Units (WMUs) sampled in the OSR is 0.17 per km$^2$. We 
...
In conclusion, moose are well studied in the OSR. Whereas no estimates of 
density have revealed a downward trend in population density, due to the 
behavioural responses of moose to human disturbance we recommend further work 
exploring changes in moose density at smaller scales in proximity to human 
disturbance.
...
+ Number, temporal, and spatial scope of Government of Alberta aerial surveys
+ Peer-reviewed literature
+ ABMI camera data and habitat modeling
+ Potentially hunter harvest surveys
...
surveys) is 0.17 animals per km$^2$. From 1993 to 2013, when the random 
...
right of the plot to remove or add-in data from a particular WMU. One outlier 
datapoint, a density estimate of 1.2 in WMU 515 in 2004, was removed from the 
plot for the sake of graphical clarity. 
...
``` {r include=TRUE, echo=TRUE, message=FALSE, warning=FALSE, eval=TRUE, fig.cap="Figure 3. Mean annual moose density in WMUs of the Oil Sands Region.", out.width="90%"}
...
### Density from ABMI remote cameras
...
confidence intervals are smaller. For the entire OSR, estimated moose densites 
were 0.68, 0.54, and 0.41 animals per km$^2$ in 2015, 2016, and 2017, 
respectively [^1].
...
vegetation 'backfilled' in the unit based on the surrounding area. Figure 12 
displays a map of the ABMI current relative abundance predictions for moose 
across the OSR, using RC data from 2015-2017. Pixels depicted in red are 
predicted to have the highest abundance for moose, whereas in areas of dark blue 
moose are expected to be less abundant or absent. 
...
Currently, moose density in the OSR is comparable to that of the rest of the 
boreal region of Canada. The years over which we summarized various population
parameters (1993-2016) correspond roughly to those over which oil and gas 
exploration and extraction in Northeastern Alberta increased significantly 
(Schindler 2010). We did not uncover any downward trends in any of these 
parameters over this time interval, and the summaries we calculated are 
characterized by variation between both years and study areas (WMUs). Plotting 
density and lambda spatially (Figures X) reveals that both values decrease with
increasing latitude (i.e., further north), which may be due to the more intense
oil and gas extraction processes north of the city of Ft. McMurray and 
disruption to predation in the area (Neilson and Boutin 2017). However, such an 
effect is not clear from this review as the density trend may be due to habitat
or other effects that existed prior to mine development.
